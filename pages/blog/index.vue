    ...
    <div class="block float-left w-full">
      ...
      <div class="block w-11/12 xl:w-8/12 mx-auto my-0">
        ...
        <div class="block float-left w-full md:pr-8 md:border-r md:border-lgrey md:w-8/12">
          <router-link :to="{ path: '/blog/'+ post.id +'/' + post.permalink}" class="cell block float-left w-full mb-12" v-for="post in displayedPosts" :key="post.id">
              <div class="block float-left w-full mb-4">
                  <h4 class="text-3xl font-bold text-black">{{post.title}}</h4>
                  <p class="text-sm mb-4 font-bold text-grey" v-html="post.date" />
                  <img class="block float-left h-auto w-full" :src="api_url + post.thumb.url" alt="post.title" />
              </div>
        ...
        <div class="hidden md:float-left md:w-4/12 md:pl-8 md:block">
          ...
          <div class="block float-left w-full mb-12">
            ...
            <router-link :to="{ path: '/blog/'+ post.id +'/' + post.permalink}" class="block float-left w-full" v-for="post in displayedPosts" :key="post.id">
              <div class="block float-left w-full">
                  <h4 class="text-lg text-grey mb-4 duration-300 hover:text-pink" v-html="post.title" />
              </div>
            </router-link>
          </div>
